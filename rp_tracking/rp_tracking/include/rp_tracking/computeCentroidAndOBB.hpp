/**
 * Copy of the computeCentroidAndOBB function in PCL version 1.14, as this is not implemented in this version
 * Altered to fit the requirements of this project: Extracts only z-rotation from rotation matrix
 */
  ...
  Eigen::Matrix<Scalar, 3, 3> &obb_rotational_matrix)
{
  ...
  // Since most robot dimensions are given with no rotation in datasheets,
  // we only consider z-axis rotation (orientation) here 
  Eigen::Vector3d euler_angles = obb_rotational_matrix.eulerAngles(2, 1, 0);
  obb_rotational_matrix = Eigen::AngleAxisd(euler_angles[0], Eigen::Vector3d::UnitZ());
